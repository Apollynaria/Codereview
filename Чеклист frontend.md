...
8. Ошибки асинхронных операций обрабатываем там, где это необходимо. Не допускаем необработанных промисов, пустых `catch` и скрытия ошибок без обоснования.
9. В компонентах убираем неиспользуемые импорты, props, переменные и методы.
10. Используем `optional chaining` вместо цепочек проверок через `&&`, если это не меняет поведение кода. Если встречаем такие проверки при доработке модуля — исправляем.
...
13. При изменении поведения модуля создаём или обновляем связанную с ним документацию в Markdown.
...
```
2. Общие компоненты и утилиты из библиотеки импортируем через `@tarif/component-lib`.

## TypeScript

1. Новый и изменённый код должен быть типизирован.
...
getAllDiffHeatSupplySystems = createAction<
  EDiffHeatSupplySystemsEndpoints.getAllDiffHeatSupplySystems,
  TDictionariesActionMethods
>({
  modulePath: 'dictionaries',
  actionName: EDiffHeatSupplySystemsEndpoints.getAllDiffHeatSupplySystems,
});
```  
